...
<div id="content">
	<div id="add-place">
		...
		<div class="wp-form-add-place">
			<div class="container">
				<form id="check_in" method="post" action="{{url('topic/edit-post')}}" >
					{{ csrf_field() }}
						...
						<div class="row">
							...
							<div class="col-lg-8 bg-white pt-4 pb-4">
								...
								<div class="form-group mt-4 ralative-live-search">
									...
									@enderror
									<input type="text" class="form-control" placeholder="Nhập tên địa điểm, chủ đề" id="place_topic" name="name_topic" autocomplete='off' value="{{ $post->topic->name }}">
									<input type="hidden" id="id_topic" name="id_topic" value="{{ $post->topic->id }}">
								...
								<div class="form-group mt-4">
									...
									<p class="form-error">{{$message}}</p>
									@enderror
									<input type="text" class="form-control" placeholder="Nhập vào tiêu đề" id="name_place" name="title" value="{{ $post->title }}">
									<!-- post id -->
...
<script>
	var exist = '{{Session::has('update_success')}}';
    if(exist){
        Swal.fire({
	        position: 'center',
	        icon: 'success',
